...
public class ReadMetadata {
    ...
    public ReadMetadata( final Set<Integer> crossContigIgnoreSet,
        ...
        coverage = (int) ((nReadBases + nRefBases/2) / nRefBases);
        ...
        for ( int idx = 0; idx != nPartitions; ++idx ) {
            ...
            final int firstStart = stats.getFirstLocation();
            final Integer lastContigID = contigNameToID.get(stats.getLastContig());
            final int lastStart = stats.getLastLocation();
    ...
    public static final class PartitionStatistics {
        ...
        public PartitionStatistics( final Iterator<GATKRead> readItr, final Map<String, String> readGroupToLibraryMap ) {
            if ( !readItr.hasNext() ) {
        ...
        private PartitionStatistics( final Kryo kryo, final Input input ) {
            ...
            while ( nEntries-- > 0 ) {
                ...
                libraryToFragmentSizeCountMap.put(readGroup, counts);
            }
            nReads = input.readLong();
